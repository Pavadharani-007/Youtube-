...
API_KEY= 'AIzaSyClD9_Z_RkyAGjwP9P6HfB3Jp3mmGSOHko'
...
def channel_stats(channel_id):
    ...
    data=dict(channel_name=response['items'][0]['snippet']['title'],
                ...
                #joined_on=convert_duration(response['items'][0]['snippet']['publishedAt'])
            )
    ch.append(data)          
    return ch
...
# FUNCTION TO GET CONVERT DURATION:    
    ...
    for i in range(0,len(videoids),50):
        ...
        for video in response['items']:
            video_details=dict(channel_name=video['snippet']['channelTitle'],
                                ...
                                publishedat=remove_t_and_z(video['snippet'].get('publishedAt'))
        ...
        try:
            ...
            for cmnt in response['items']:
                comment_data=dict(channel_id=cmnt['snippet']['topLevelComment']['snippet']['channelId'],
                                    ...
                                    publishedAT=remove_t_and_z(cmnt['snippet']['topLevelComment']['snippet']['publishedAt']),
                                    updated=remove_t_and_z(cmnt['snippet']['topLevelComment']['snippet']['updatedAt']),
...
# FUNCTION TO GET MAIN FUNCTION DETAILS:    
...
def sql_main(md):
    sql_c='''INSERT INTO channeldata(channel_name,channel_id,total_videos,subscribers,views )values(%s,%s,%s,%s,%s)'''
...
if selected == "Data Analysis":     
    ...
    if questions == '1. What are the names of all the videos and their corresponding channels?':
        ...
        mycursor.execute("SELECT video_name,channel_name,likes_count,dislikes_count  FROM videodata  ORDER BY likes_count+dislikes_count DESC ")
        out = pd.DataFrame(mycursor.fetchall(),columns=mycursor.column_names)
        st.write(out)
